...
export function generateMockTrainingSessions(days: number = 30): TrainingSession[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    sessions.push({
      ...
      sessionType,
      overallFeeling: Math.max(1, Math.min(100, Math.round((baseLevel + variance()) * 10))),
      forehandRating: Math.max(1, Math.min(100, Math.round((baseLevel + variance() + 1) * 10))),
      backhandRating: Math.max(1, Math.min(100, Math.round((baseLevel + variance()) * 10))),
      serveRating: Math.max(1, Math.min(100, Math.round((baseLevel + variance() + 0.5) * 10))),
      receiveRating: Math.max(1, Math.min(100, Math.round((baseLevel + variance() - 0.5) * 10))),
      generalRating: Math.max(1, Math.min(100, Math.round((baseLevel + variance()) * 10))),
