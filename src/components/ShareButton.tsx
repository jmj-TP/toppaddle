...
export default function ShareButton({ racketName, score, price, isCustom, className }: ShareButtonProps) {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        ...
        // User cancelled or share failed, show dialog as fallback
        if ((error as Error).name !== 'AbortError') {
          setShowDialog(true);
        }
      }
    } else {
      // Show custom dialog for browsers without native share support
      setShowDialog(true);
    }
  };
  ...
  const shareLinks = {
    ...
    email: `mailto:?subject=${encodeURIComponent('Check out my table tennis recommendation!')}&body=${encodeURIComponent(`${shareText}\n\n${shareUrl}`)}`,
  };
    ...
    <>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Share Recommendation</DialogTitle>
            <DialogDescription>
              Share your table tennis recommendation with friends
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button
                ...
                className="justify-start"
                onClick={() => {
                  window.open(shareLinks.reddit, '_blank');
                  setShowDialog(false);
                }}
              >
                <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor">
